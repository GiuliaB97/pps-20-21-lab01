 ...
 * where the following element of the last one into the list
...
public interface CircularList {
     ...
     * Example: having a list like {1,2,3} the first call of next() returns 1,
     * the second call returns 2, then 3 is returned at the third call.
     * Finally the fourth one return 1 again,
     ...
     * Its behaviour is dual of the next() method.
     * @return the previous element into the list
     */
    Optional<Integer> previous();

    /**
     * Reset the current element to the first one of the list.
     * The first one is the first added to the list.
     */
    void reset();
     ...
     * (e.g., the next even element, the next odd element, etc.)
